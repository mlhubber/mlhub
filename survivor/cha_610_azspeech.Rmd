...
The **synthesize* command will generate spoken word audio from
supplied text and play the audio on the system's default audio
output. With `-o` or `--output` a *wav* file can be specified as the
output rather than having the audio played through the speakers.

```console
$ ml synthesize azspeech 		  \
     [(--file|-f) <txt file>] 	  \
	 [(--lang|-l) <lang>]		  \
	 [(--voice|-v) <voice>]       \
	 [(--output|-o) <output.wav>] \
...
computer microphone for up to 15 seconds and then transcribe it to
standard output. The command can also be used to transcribe speech
from an audio file (*wav* only) into text.
