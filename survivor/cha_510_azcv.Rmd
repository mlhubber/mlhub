...
cloud API to access closed source pre-built models for 
`r ai_chapter("computer vision", "computer-vision")`
tasks. The package provides a demonstration, a graphical user
interface, and command line tools that utilise the pre-built models.
...
portal. These will be prompted for when running a command, and then
saved to file to reduce the need for repeated authentication requests.

Most of the commands provided by the package take an image as a
parameter which may be a URL or a path to a local file. For brevity
through this chapter the URLs we use are short URLs generated through
...
In addition to the `r ml_command(azcv, demo)` and
`r ml_command(azcv, gui)` commands the package supports many 
computer vision operations including 
`r ml_command(azcv,adult)`,
`r ml_command(azcv,brands)`,
`r ml_command(azcv,category)`,
`r ml_command(azcv,celebrities)`,
`r ml_command(azcv,color)`,
`r ml_command(azcv,describe)`,
`r ml_command(azcv,faces)`,
`r ml_command(azcv,landmarks)`,
`r ml_command(azcv,objects)`,
`r ml_command(azcv,ocr)`,
`r ml_command(azcv,tags)`,
`r ml_command(azcv,thumbnail)`, and
`r ml_command(azcv,type)`.
...
service also sends your image files to Azure for analysis.*

<!---------------------------------------------------------------------->
## azcv quick start
<!---------------------------------------------------------------------->

Here are a collection of sample commands available in the package. Do
note that you need to obtain an Azure Cognitive Services API key
(Chapter \@(azcv)), then copy an example command from below to paste
and then run it on your Ubuntu command line. The examples are expanded
upon through the following sections.
...
<!-- https://images.all-free-download.com/images/graphiclarge/marina_bay_sands_singapore_559899.jpg -->

<!-- ml landscape azcv https://bit.ly/3vnuG24 FAILS -->
<!-- https://bit.ly/3vnuG24 -->
...
To run a simple interactive graphical user interface (GUI):
...
The sample hand writing used here to demonstrate the OCR button was
supplied by Ryley Nolan and Grace Patterson (used and accredited with
permission). The accuracy is pretty good!


The GUI is basic but functional. You can enter a path to a local image
file or a URL to an image in the text entry, or browse to a local
image file. By default the Browse begins in a folder of sample images
for you to try out.
...
With even more confidences, Uluru is also described:
...
```{r echo=FALSE, cache=TRUE, fig.align="left", out.width="40%"}
include_graphics_url("https://bit.ly/3u3nwhW")
```
...
``` console
$ ml ocr azcv https://bit.ly/2Op1qYk | cut -d, -f2-
...
I love my cat and wouldn't change a thing about him!
```

```{r echo=FALSE, cache=TRUE, fig.align="left", out.width="50%"}
include_graphics_url("https://bit.ly/319WJUM")
```

<!-- https://github.com/gjwgit/azcv/raw/master/docs/handwritting_malala.jpg -->
<!-- http://www.handwrittenocr.com/images/Handwriting/16.jpg -->

``` console
$ ml ocr azcv https://bit.ly/319WJUM
