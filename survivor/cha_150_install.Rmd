...
The various options for installing Ubuntu are covered in this chapter,
finishing with the installation of MLHub itself in Section
\@ref(installing-mlhub).
...
Ubuntu server in the cloud for just $5 per month. 

You can get free credit ($200) from Microsoft to trial Azure.  After
signing up, from the portal add a new Ubuntu 20.04 LTS server,
creating a new resource group for it, providing a name for the virtual
machine, a user name and a password, choosing a location (e.g.,
westus2), opening network port 22 for ssh connectivity, and choosing a
machine size to suit your needs. Once created go to the portal and
provide a DNS name from the overview page’s, DNS link. Connect to the
server using `r Command(ssh)` from a terminal to the DNS name, set the
server up as in Section \@ref(installing-mlhub), then connect to the
server using X2Go on your own desktop.
