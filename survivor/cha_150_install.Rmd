...
overview page’s, DNS link. Connect to the server using 
`r Command(ssh)` from a terminal to the DNS name, set the server up as in
...
requires the MATE desktop and the X2Go server to be installed on the
cloud server. This can take 15 minutes or so.

```bash
$ wajig install ubuntu-mate-desktop x2goserver
```

Whilst installing select the default option for any questions (e.g.,
gdm3).
